	...
	let loadProducts = data.then(x => {
		for (var i = 0; i < data.products.length; i++) {
			mainRow.append('<div class="col-md-3" id=' + data.products[i].id + '><div class="card card-product card-plain"><div class="card-image">'
				+ '<a href="#" data-toggle="modal" data-target="#detailedDishes' + '-' + data.products[i].id + '" onclick=openNav()>'
	...
	let loadProducts = data.then(x => {
		for (var i = 0; i < data.products.length; i++) {
			mainRow.append(
				...
				+ '<br>'
				+ '<center>'
				+ '<button type="button" class="btn btn-danger add-to-cart-detail">Thêm vào giỏ hàng</button>'
				+ '</center>'
				+ '</div>'
				+ '</div>'
				+ '</div>'
	...
	let loadProducts = data.then(x => {
		for (var i = 0; i < data.menu.length; i++) {
			mainRow.append(
				...
				+ '<div class="btn-group">'
				+ '<button class="btn btn-sm btn-border btn-round" onclick="minus(this)">-</button>'
				+ '<button class="btn btn-sm btn-border btn-round" onclick="add(this)">+</button>'
				+ '</div>'
				+ '</td>'
				+ '<td>'
